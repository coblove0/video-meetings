...
- `src/app/page.tsx` — the protected home page. Client component: on mount it reads `accessToken` from `localStorage` and redirects to `/auth/login` if absent (there's no server-side session/middleware, so this is a client-side-only guard — see § Auth below). Fetches `GET /users/me` and `GET /meetings` in parallel (Bearer token); the header renders the current user as a HeroUI `Avatar` (backed by `useAvatarUrl`, see below, with `Avatar.Fallback` initials — same derivation as the profile page — shown until/unless a real avatar loads) plus their name (or email if no name is set) as a `Link` to `/profile` — this is the only way to reach the profile page from the UI. The body renders the total meeting count and the 3 most recent meetings (sorted client-side by `date` descending — the API returns the full unsorted array, no pagination) as links to `/meetings/[id]`. A 401 from either request clears the stored token and redirects to `/auth/login`; any other non-OK response or network failure shows a retryable `Alert`. The "Create meeting" button is currently a disabled placeholder — there is no meeting-creation UI yet.
- `src/app/profile/page.tsx` — the profile page. Client component, same `accessToken` guard as the other protected pages. On mount fetches `GET /users/me` (Bearer token); a 401 clears the token and redirects to `/auth/login`, any other non-OK response or network failure shows a retryable `Alert`. Renders the email (read-only) and name inside a HeroUI `Card`, with a HeroUI `Avatar` (see `useAvatarUrl` below) falling back to `Avatar.Fallback` initials derived from the name (first letter of up to the first two words) or, if no name is set, the first letter of the email. Below the avatar, a file `Input` (`accept="image/jpeg,image/png"`, remounted via a `key` counter after a successful upload to reset it, same uncontrolled-input pattern as the meeting file upload) plus an "Upload avatar" button prevalidate the picked file's MIME type/size client-side against the same limits as the API (5 MB, JPEG/PNG) before sending it via `XMLHttpRequest`+`FormData` (for `xhr.upload.onprogress`, rendering a HeroUI `ProgressBar`) to `POST /users/me/avatar`; on success it bumps a `avatarVersion` counter that `useAvatarUrl` watches to refetch the new image without a page reload, and on failure (413/415/401/other) shows an inline `Alert` distinct from the save-name and load-failure alerts, leaving the currently-displayed avatar unchanged. The name is editable via a HeroUI `Form`/`TextField` (bound to local state, seeded from the fetched profile) with a "Save" button; submitting sends `PATCH /users/me` (Bearer token, `{ name }`) and updates both the input and the avatar initials straight from the response body, without reloading or refetching. A 401 on save clears the token and redirects to `/auth/login` like the initial load; other failures (non-OK response or network error) show an inline `Alert` above the field without clearing the typed value, distinct from the retryable top-level `Alert` used for load failures. Below that, a separate "Change password" HeroUI `Card`/`Form` renders "Current password"/"New password" `TextField`s (`autoComplete="current-password"`/`"new-password"`, same `InputGroup` show/hide-eye-icon toggle pattern as `src/app/auth/login/page.tsx`), client-validating the new password is non-empty resp. at least 8 characters (`NEW_PASSWORD_MIN_LENGTH`, mirroring the API's `ChangePasswordDto`) before submit sends `POST /users/me/password` (Bearer token, `{ currentPassword, newPassword }`). A 401 clears the token and redirects to `/auth/login` like the rest of the page; a 400 (wrong current password — the API's `ChangePasswordHandler` throws `BadRequestException('Current password is incorrect')`) is handled locally rather than falling into the 401 redirect path, surfacing the server's message inline via the same message-parsing pattern as `src/app/register/page.tsx` (array-or-string `message`, `.json()` guarded against non-JSON bodies) without clearing either field or the stored token; any other non-OK response or network failure shows a generic inline error, also without clearing the fields. Only on success are both fields cleared and a "Password changed." success `Alert` shown.
- `src/hooks/use-avatar-url.ts` — `useAvatarUrl(hasAvatar, version?)`, shared by `src/app/page.tsx` and `src/app/profile/page.tsx` so the blob-URL fetch/lifecycle logic (see § Architecture note below) lives in one place instead of two near-identical `useEffect`s. Fetches `GET /users/me/avatar` (Bearer token) whenever `hasAvatar` or `version` changes, turns a successful response into an `Object URL` via `URL.createObjectURL`, and returns it (or `null` if the user has no avatar, matching the API's 404-when-`hasAvatar`-is-false behavior — callers show `Avatar.Fallback` in that case). A 401 clears the token and redirects to `/auth/login`, same convention as every other authenticated call in this app. The new URL is always created and returned _before_ the previous one is revoked (not the other way around), so a caller's `<Avatar.Image src>` is never left pointing at an already-revoked blob URL while a replacement (e.g. a fresh upload bumping `version`) is in flight; the very last URL is revoked on unmount.
...
`@playwright/test` is configured for e2e testing: `playwright.config.ts` (project root `apps/web`) drives Chromium against `http://localhost:3000`, always reusing an already-listening dev server (`reuseExistingServer: true` unconditionally — under autonomous/CI-less runs a missed reuse-race spawns a second `npm run dev` tree and piles up orphaned Next.js build workers on port 3000). Specs live under `e2e/*.spec.ts`. Tests seed their own data (register a user, create a meeting, upload a file) via direct API calls through Playwright's `request` fixture rather than the UI, then drive only the flow under test through the browser — this requires `apps/api`'s dev server and a local Postgres to be running (see root `CLAUDE.md` § Database), same as `apps/api`'s own e2e suite. Run via `npm run test:e2e -w web`.

**Windows path-casing gotcha:** on machines where the repo's real on-disk path differs in case from what shells report as `cwd` (e.g. the drive/folder is canonically `D:\PS\...` but every shell reports `D:\ps\...` — check with `node -e "console.log(require('fs').realpathSync.native('.'))"` vs `process.cwd()`), invoking `playwright test` through `npx`/the plain `playwright` bin makes Node's default module resolution realpath-normalize some internal `@playwright/test` requires but not others, instantiating its `lib/globals.js` suite-tracking state twice — every spec then fails at collection with `Playwright Test did not expect test.describe() to be called here`, even trivial ones, regardless of file content (not a real duplicate-package issue; `npm ls @playwright/test playwright` will show a single deduped copy). `test:e2e` in `package.json` works around this unconditionally by invoking `node --preserve-symlinks ../../node_modules/playwright/cli.js test` instead of the `playwright` bin, which skips the realpath step and keeps a single consistent module instance. This is harmless on machines/CI without the casing mismatch, so don't revert it back to a plain `playwright test` script. Note `--preserve-symlinks` disables Node's realpath-based dedup for every module in that process, not just `@playwright/test` — currently safe since the two apps share no code/packages (see root `CLAUDE.md`), but if this repo ever grows an internal shared workspace package symlinked into `node_modules`, re-check that this flag doesn't reintroduce the same class of duplicate-module-instance bug for that package.
