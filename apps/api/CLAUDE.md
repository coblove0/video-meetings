...
- `src/users/` — `UsersModule` owns the `User` entity: creating, looking one up by email, and reading/updating the current user's profile, all via Prisma, exposed to HTTP via `UsersController` (`GET`/`PATCH /users/me`, guarded by `@UseGuards(JwtAuthGuard)`, reading the caller via `@CurrentUser()`) which dispatches through `CommandBus`/`QueryBus` like the other feature controllers. It doesn't get imported by `AuthModule` or anything else — `AuthModule` only reaches it through the CQRS buses — but it does import `AuthModule` itself to get `JwtAuthGuard` into scope for its own controller, same pattern as `MeetingsModule`.
  ...
  - `commands/impl/upload-avatar.command.ts` + `commands/handlers/upload-avatar.handler.ts` (`@CommandHandler`) — takes a `userId` and the uploaded `Express.Multer.File` (already written to disk by multer's `diskStorage` before this runs). Reads the file's own bytes back via `fs/promises.readFile` and checks them against the JPEG (`FF D8 FF`) and PNG (`89 50 4E 47 0D 0A 1A 0A`) magic-byte signatures — a check on top of multer's `fileFilter`, which only trusts the client-supplied `Content-Type`; a mismatch deletes the just-written file and throws `UnsupportedMediaTypeException`. Then looks up the caller's current `avatarPath`, 404s via `NotFoundException` (deleting the orphaned upload first) if the user no longer exists, writes the new `avatarPath`/`avatarMimeType` to the DB, and only _after_ that DB write deletes the old avatar file (if any) — so a crash between the two steps never leaves the DB pointing at a missing file. Returns a `UserProfileResponse`, same shape as the other profile handlers.
